...
export default function AccordionCategories({ title, description, hasPromotion, children }: AccordionProps) {
  ...
  return (
    <div className='mb-4'>
      <button
        className='w-full bg-white p-4 px-[16px] py-[30px] text-start transition-colors hover:bg-gray-50'
        onClick={() => setIsOpen((prev) => !prev)}>
        <div className='flex items-center justify-between'>
          <div className='flex gap-1.5'>
            <span className='font-bold text-neutral-900'>{title}</span>
            {hasPromotion && <Image src='/icons/circle-dollar.svg' alt='Item em promoção' width={18} height={18} />}
          </div>
